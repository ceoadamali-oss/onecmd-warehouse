...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. Fetch active items from Square Catalog to map Variation IDs to SKUs
    let catalogItems = [];
    let cursor = null;
    do {
    ...
    // 3. Fetch physical scan inventory counts
    let allInvRows = [];
    let start = 0;
    const pageSize = 1000;
    while (true) {
    ...
    // 4. Fetch orders since scan completion
    ...
    for (const order of orders) {
      ...
      for (const item of lineItems) {
        ...
        // Specific overrides if still unmatched
        if (!matchedProduct && item.name) {
